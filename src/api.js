...
export const fetchTriggers = (marketId) => api(`/api/triggers${marketId ? `?marketId=${marketId}` : ''}`)
export const fetchWorldState = (marketId) => api(`/api/world-state${marketId ? `?marketId=${marketId}` : ''}`)
export const fetchSwarmRuns = (marketId) => api(`/api/swarm-runs${marketId ? `?marketId=${marketId}` : ''}`)
